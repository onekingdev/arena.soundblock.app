...
@Component({
  selector: 'app-track-previews',
  templateUrl: './track-previews.component.html',
  styleUrls: ['./track-previews.component.scss'],
})
export class TrackPreviewsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  get regionStart(): string {
    return this.trackRegion.start.toFixed(2).replace('.', ':');
  }
  ...
  get regionEnd(): string {
    return this.trackRegion.end.toFixed(2).replace('.', ':');
  }
  ...
  ngAfterViewInit() {
    ...
    // If the track is playing and user drags the preview region start playing from the beginning
